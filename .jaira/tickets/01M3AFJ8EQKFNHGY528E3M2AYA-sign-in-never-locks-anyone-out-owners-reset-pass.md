...
updated-at: 2026-09-24T20:28:38Z
updated-by: Claude
claimed-by: vm-12405
...
review-summary: "Independent review 8 of 19db5e8 (parent caf261a), 2026-09-24. The change does what Aly decided: nobody is locked out and a failed sign-in returns 401 'Incorrect email or password.'; any owner resets any active user's password except the primary's (403 'Only the primary owner can change the primary owner's password.', audited as owner_change_blocked, capped notice); 8-character minimum; primary-only powers unchanged. Reset is atomic under BEGIN IMMEDIATE with a fresh re-read of the actor and the primary flag; target sessions revoked (own reset keeps the current session); failed sign-ins cleared; password_reset user event 'in app'; target notified and other owners when the target is an owner; the CLI shares the code. No path lets a secondary or non-owner reset the primary; ids match exactly; non-owner check precedes 404 (no existence leak); keep_session cannot come from the client; no plaintext passwords in user_events or notifications. Browser check: the new People form matches 'Add a user'. Full suite 452 OK; 13 of 15 mutations caught."
review-gaps: "Fixed in ee84113 (not re-reviewed; full suite Ran 456 tests, OK): M1 the 90-day prune scanned login_attempts on every attempt and emails were stored uncapped -> prune at most hourly per process, emails capped at 320 (an attempted_at index is a follow-up for the next schema bump). L1 unknown/inactive email skipped PBKDF2 (timing oracle) -> checked against a fixed dummy hash. L2 mutations B (in-transaction actor re-read) and K (canResetPassword) survived -> stale demoted/deactivated secondary test and a node-evaluated rule test. L3 own-reset said 'Give them', history did not refresh, row links wrapped -> 'Your password has been changed.', re-render after reset, actions grouped on one line (screenshots in the lock 8 scratch folder). L4 blocked row and notice did not read well -> 'Blocked password reset of the primary owner' and 'blocked password reset of the primary owner by <actor>'. L5 non-string passwords were str()-converted and accepted -> 400 'Password must be text.'. I1 stale 'login throttle' comment -> fixed. Left as is (pre-existing or accepted): I2 wrong method/unknown sub-path returns HTML 404 and a non-object JSON body returns 500, as for all routes; I3 _require_user validates the session before BEGIN IMMEDIATE (harmless race, all endpoints); I6 an inactive primary gets 400 'inactive' rather than an audited 403 (no write happens); I7 pre-existing CSP inline-style console error. Accepted by Aly: unlimited online password guessing (rate limiting for hosting belongs on 6BXYJZ)."
review-verdict: "approve with follow-ups; M1, L1-L5, I1 fixed in ee8411383eb5270244f47422c782d4f17a8cc392; fixes not re-reviewed"
review-check: "1. Windows, repo root: .venv\\Scripts\\python.exe tests\\run.py; expect 'Ran 456 tests' and 'OK'. 2. Start Astra, sign in with a wrong password 10 times, then the right one: signed in, each wrong try said 'Incorrect email or password.'. 3. As the primary, People screen: every active user row has 'Reset password'; reset a member's password with an 8-character password typed twice; the teal message says to give them the new password; the member signs in with it and their old session is gone; Owner access history shows 'Password reset: <member> by <you> · in app'. 4. Reset your own password: 'Your password has been changed. Your other sessions were signed out.' and you stay signed in. 5. Sign in as a secondary owner: the primary's row has no 'Reset password'; other rows do. 6. Try a 7-character password: refused."
---
...
- **2026-09-24 20:28 · Claude** — Review 8 recorded (approve with follow-ups). M1, L1-L5, I1 fixed in ee84113, not re-reviewed; I2, I3, I6, I7 left as is.
