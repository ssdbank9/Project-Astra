...
updated-at: 2026-09-23T15:54:04Z
updated-by: Claude
claimed-by: vm-28497
...
- [x] On completed, cancelled and abandoned tasks renderDetail hides or disables the Edit, criticality, dependency and reviewer forms and shows a line saying to reopen the task first; attachments, final-result marking and reopen stay available
  proof: src/astra/static/app.js:607 (closedNote), :612 (editForm), :789 (reopen on all closed); tests/test_web.py:1219 test_closed_task_hides_refused_forms_and_says_reopen_first, :1238 test_closed_task_keeps_attachments_and_final_result_marking_for_the_owner
- [x] On a submitted task the Status select is locked; the dialog points to accept / request changes
  proof: src/astra/static/app.js:610 (disabled + aria-describedby status-locked-hint); tests/test_web.py:1264 test_submitted_task_locks_status_and_points_to_the_decision
- [x] On a terminal task a Manager's Status select offers only statuses that can become a request (back to ordinary work), never cancelled, abandoned or changes_requested
  proof: src/astra/static/app.js:539 BACK_TO_WORK, :612 request form; tests/test_web.py:1254 test_manager_on_a_closed_task_may_only_request_ordinary_work, :1249 test_owner_on_a_closed_task_gets_no_status_select
- [x] Static JS test in tests/ covers the rendered markup for each case; node --check src/astra/static/app.js clean; full suite green
  proof: tests/test_web.py:1199 AstraDetailDialogStatusGateTests (renders renderDetail under node, 6 tests); node --check clean; tests/run.py Ran 275 tests OK
